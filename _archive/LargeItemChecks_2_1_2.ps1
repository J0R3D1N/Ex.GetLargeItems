...
"@
    ...
    elseif($inplacehold)
	{
		Try
        {
	        ...
	        Write-Log ("====>  MAILBOX: {0} | Found {1} folders (RECOVERABLE ITEMS FOLDERS)" -f $MBX,$Folders.TotalCount);
        }
	    Catch [Microsoft.Exchange.WebServices.Data.ServiceResponseException]
	    {
            $myError = @"
...
Try
{
	...
	Write-Log "====>  Determining if Exchange Web Services 2.2 is installed"
    $CheckWebServicesInstalled = Test-Path -Path 'HKLM:\SOFTWARE\Microsoft\Exchange\Web Services\2.2'
	
	if (!$CheckWebServicesInstalled)
	{
        [System.Console]::ForegroundColor = "Yellow"
        Write-Host "`n`r||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||"
        Write-Host "||||||||||`n`r||||||||||  Exchange Web Services API cannot be found or is not installed.`n`r||||||||||  Download EWS API 2.2 here: http://www.microsoft.com/en-us/download/details.aspx?id=35371`n`r||||||||||"
        Write-Host "||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||";
        [System.Console]::ResetColor()

		Write-Log "====>  WARNING  <==== | Exchange Web Services API 2.2 was not found, Script will now exit"
        Exit 1
	}
