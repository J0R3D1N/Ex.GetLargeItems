...
<#
.SYNOPSIS
This script will run a series of cmdlets / functions using Exchange Web Servers
to search mailboxes for items over a specified size. Useful for Office 365
engagements where you need to remediate large items before migration.

.DESCRIPTION
This script arranges building-block cmdlets / functions to connect to an Exchange environment and loops through all or a subset of mailboxes with an impersonator account using Exchange Web Services API.  The impersonator account will enumerate every item in every folder and identify items that are exceeding a specific size. The script is designed to be exectued before a cross-forest migration or anytime an Organization needs to report, export, or move items that may not be compliant with an organization or target organization item size quota / limitation. This script is most commonly used in conjunction with Office 365 migrations due to the 25MB item size limitation.
...
.PARAMETER ImportCSV
Specifies the CSV file to be used as the source of mailboxes to search through. The CSV file should not contain any headers and should ONLY contain the primary SMTP address for the users you want to process.
...
.PARAMETER LargeItemNotification
...
When running the script with no parameters, it will prompt for any values which are mandatory.  When it script completes, it will display the location for the logfile which will provide a detailed account of what the script did during the last time it ran.

Try not to run the script without any parameters, especially if you are proccessing ALL or a lot of large mailboxes because it won't give you any valuable output for the amount of time it took for it to complete.

.EXAMPLE
LargeItemChecks_2_0.ps1 -ServiceAccountDomain <Domain> -ServiceAccountName <User> -ServicePassword <Password> -ItemSizeLimit <Value in MB> -CreateCSV

In this example the script will prompt for all mandatory parameters, but will create a CSV file which will contain all the item violations from all the mailboxes that were scanned.

From this CSV you can create your CSV file as the source input for the mailboxes to scan to provide a more efficent processing the next time you run the script.
...
#>
    Write-Log ("====>  MAILBOX: {0} | Creating Impersonation session using Exchange Web Services" -f $MBX)
...
'@ 
	...
	{
        Write-Log ("====>  MAILBOX: {0} | Exchange Web Services Version: Exchange2010_SP1" -f $MBX)
	...
	{
        Write-Log ("====>  MAILBOX: {0} | Exchange Web Services Version: Exchange2007_SP1" -f $MBX)
...
Function Create-LargeItemReport
{
    ...
    Write-Log ("====>  Attempting to create CSV file: {0}\{1}" -f $ScriptPath,$OutputFile);
...
Function Export-LargeItems($PSTPath,$FolderName,$MBX,$HomeDir)
{
    if ($CentralizedExport)
    {
        ...
        else
        {
            ...
            New-MailboxExportRequest -Name $MBX -Mailbox $MBX -FilePath "$PSTPath\$SamAccountName-LargeItems.pst" -IncludeFolders $FolderName -Confirm:$False;
        }
    }
    else
    {
        ...
        else
        {
            ...
            New-MailboxExportRequest -Name $MBX -Mailbox $MBX -FilePath "$HomeDir\$SamAccountName-LargeItems.pst" -IncludeFolders $FolderName -Confirm:$False;
        }
    }
}
...
Function Send-LargeItemNotice($Mailbox,$Recipient,$PSTPath,$HomeDir,$LargeItemFolder)
{
    ...
    $O365Admin = "Office 365 Migration Team <office365@autotradergroup.com>"
    ...
    Send-MailMessage -Body $MessageBody -BodyAsHtml -From $O365Admin -SmtpServer $SMTPServer -Subject "Large E-mail Attachments Auto-Archive is Complete" -To $Recipient -Bcc "office365@autotradergroup.com","japarker@microsoft.com"
...
Function Process-CSVMailboxes()
{
    ...
    foreach ($MBX in $Mailboxes)
    {
		...
		Try
		{
            ...
            Write-Log ("====>  MAILBOX: {0} | Database: {1}" -f $MBX,$MBXStats.Database)
            Write-Log ("====>  MAILBOX: {0} | Total Item Count: {1}" -f $MBX,$MBXStats.ItemCount)
            Write-Log ("====>  MAILBOX: {0} | Total Item Size: {1}" -f $MBX,$MBXStats.TotalItemSize)
            Write-Log ("====>  MAILBOX: {0} | Deleted Item Size: {1}" -f $MBX,$MBXStats.TotalDeletedItemSize)
            Write-Log ("====>  MAILBOX: {0} | Last Logon Time: {1}" -f $MBX,$MBXStats.LastLogonTime)

            #Write-Log ("====>  MAILBOX: {0} | Collecting Active Directory Attributes" -f $MBX);
            Write-Log ("====>  FUNCTION  <==== | Get-ADAttributes -DistinguishedName $DN")
            $ADInfo = Get-ADAttributes -DistinguishedName $DN;
            [System.String]$HomeDir = $ADInfo.homedirectory
            ...
            {
                #$san = "irv-san-na1"
                #$HomeDir = $HomeDir -replace "corpshare",$san
                Write-Log ("====>  MAILBOX: {0} | Home Directory set to '{1}'" -f $MBX,$HomeDir)
            }
		    ...
		    {
                #Write-Log ("====>  MAILBOX: {0} | Connecting to EWS as {1}" -f $MBX,$ServiceAccountName);
                Write-Log ("====>  FUNCTION  <==== | Get-Impersonator -Identity $MBXSMTPAddress -ImpersonatorAccountName $ServiceAccountName -ImpersonatorAccountPassword $ServicePassword -ImpersonaterAccountDomain $ServiceAccountDomain")			    
                $Service = Get-Impersonator -Identity $MBXSMTPAddress -ImpersonatorAccountName $ServiceAccountName -ImpersonatorAccountPassword $ServicePassword -ImpersonaterAccountDomain $ServiceAccountDomain;
		    }
		    else
		    {
            ...
            #Write-Log ("====>  MAILBOX: {0} | Enumerating Folders" -f $MBX);
            Write-Log ("====>  FUNCTION  <==== | Get-Folders -Service $Service")
            $Folders = Get-Folders -Service $Service;
            ...
            if (!$EXPORTONLY)
            {
                ...
                {
                    #Write-Log ("====>  MAILBOX: {0} | Validating / Creating the Large Item Folder" -f $MBX);
                    Write-Log ("====>  FUNCTION  <==== | Create-Folder -FolderName $FolderName -Service $Service")
                    Create-Folder -FolderName $FolderName -Service $Service
                }

                #Write-Log ("====>  MAILBOX: {0} | Enumerating Items" -f $MBX);
                Write-Log ("====>  FUNCTION  <==== | Get-Items -ItemSizeLimit $ItemSizeLimit -Folders $Folders -Service $Service")
                $ItemViolationLog = Get-Items -ItemSizeLimit $ItemSizeLimit -Folders $Folders -Service $Service;
                ...
                if ($ItemViolationLog)
                {
                    ...
                    {
                        #Write-Log ("====>  MAILBOX: {0} | Exporting Large Item Folder into PST" -f $MBX)
                        Write-Log ("====>  FUNCTION  <==== | Export-LargeItems -PSTPath $PSTPath -FolderName $FolderName -MBX $MBX -HomeDir $HomeDir")
                        Export-LargeItems -PSTPath $PSTPath -FolderName $FolderName -MBX $MBX -HomeDir $HomeDir
                        
                        if ($LargeItemNotice)
                        {
                            #Write-Log ("====>  MAILBOX: {0} | Generating Large Item Notification" -f $MBX)
                            Write-Log ("====>  FUNCTION  <==== | Send-LargeItemNotice -Mailbox $MBX -Recipient $MBXSMTPAddress -PSTPath $PSTPath -HomeDir $HomeDir -LargeItemFolder $FolderName")
                            Send-LargeItemNotice -Mailbox $MBX -Recipient $MBXSMTPAddress -PSTPath $PSTPath -HomeDir $HomeDir -LargeItemFolder $FolderName
                        }
                    }
                }
...
Function Process-Mailbox($Mailbox)
{
	...
	Try
	{
		...
		{
			#Write-Log ("====>  MAILBOX: {0} | Connecting to EWS as {1}" -f $MBX,$ServiceAccountName);
            Write-Log ("====>  FUNCTION  <==== | Get-Impersonator -Identity $MBXSMTPAddress -ImpersonatorAccountName $ServiceAccountName -ImpersonatorAccountPassword $ServicePassword -ImpersonaterAccountDomain $ServiceAccountDomain -Uri $Uri")
            $Service = Get-Impersonator -Identity $MBXSMTPAddress -ImpersonatorAccountName $ServiceAccountName -ImpersonatorAccountPassword $ServicePassword -ImpersonaterAccountDomain $ServiceAccountDomain -Uri $Uri;
		}
...
Write-Host "`n`r--------------------------------------------------------------------------------------------------------"
Write-Host ("----------`n`r----------  Title:    Office 365 Large Item Script`n`r----------  Purpose:  Find items in mailboxes over {0} MB and perform an action`n`r----------  Actions:  -CreateCSV, -MoveLargeItems, -ExportLargeItems`n`r----------  Script:   LargeItemChecks_2_0.ps1" -f $ItemSizeLimit)
Write-Host "----------`n`r--------------------------------------------------------------------------------------------------------`n`r"
[System.Console]::ResetColor()
...
Try
{
	...
	{
        Write-Log "====>  Scripting running from a non-Exchange PowerShell Session..."
        Write-Log "====>  Checking for Microsoft Exchange Server Management Snapins..."

        if (!(Get-PSSnapin $SnapinToLoad -ErrorAction SilentlyContinue))
        {
            Write-Log "====>  Microsoft Exchange Server Management Snapin not found, loading the Snapin..."
            ...
            {
                Write-Log "====>  Loading Microsoft Exchange Server Management Remote PowerShell..."
        ...
        {
            Write-Log "====>  FOUND Microsoft Exchange Server Management Snapin..."
        }

	}
    else
    {
        Write-Log "====>  Script running from Exchange PowerShell Session, no Snapins to load..."
    }
