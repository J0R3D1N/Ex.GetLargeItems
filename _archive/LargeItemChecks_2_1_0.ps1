...
.EXAMPLE
LargeItemChecks_2_1_0.ps1 -ServiceAccountDomain <Domain> -ServiceAccountName <User> -ServicePassword <Password> -ItemSizeLimit <Value in MB> -CreateCSV
...
.EXAMPLE
LargeItemChecks_2_1_0.ps1 -ServiceAccountDomain <Domain> -ServiceAccountName <User> -ServicePassword <Password> -ItemSizeLimit <Value in MB> -ImportCSV .\users.csv -MoveLargeItems -ExportLargeItems -LargeItemNotice
...
.EXAMPLE
LargeItemChecks_2_1_0.ps1 -ServiceAccountDomain <Domain> -ServiceAccountName <User> -ServicePassword <Password> -ItemSizeLimit <Value in MB> -ExportLargeItems -ExportOnly
...
Function Get-Folders($Service)
{
	...
	if($archiveCheck)
	{
	    ...
	    {
            [System.Console]::ForegroundColor = "Red"
            Write-Host ("`n`r====>  MAILBOX: {0} | Check service account impersonation rights" -f $MBX)
            [System.Console]::ResetColor()
...
Write-Host "`n`r--------------------------------------------------------------------------------------------------------"
Write-Host ("----------`n`r----------  Title:    Office 365 Large Item Script`n`r----------  Purpose:  Find items in mailboxes over {0} MB and perform an action`n`r----------  Actions:  -CreateCSV, -MoveLargeItems, -ExportLargeItems`n`r----------  Script:   LargeItemChecks_2_1_0.ps1" -f $ItemSizeLimit)
Write-Host "----------`n`r--------------------------------------------------------------------------------------------------------`n`r"
[System.Console]::ResetColor()
