...
<#
.SYNOPSIS
This script will run a series of cmdlets / functions using Exchange Web Services to search mailboxes for items over a specified size. Useful for Office 365 on-boarding where you need to remediate large items before migration.

.DESCRIPTION
This script arranges building-block cmdlets / functions to connect to an Exchange environment and loops through all or a subset of mailboxes with an impersonator account using Exchange Web Services API.  The impersonator account will enumerate every item in every folder and identify items that are exceeding a specific size. The script is designed to be executed before on-boarding an Organization to Office 365.
...
Primary:
The script will target the users primary mailbox and folders (MsgFolderRoot)

Archive:
The script will target the users archive mailbox and folders (ArchiveMsgFolderRoot)

RecoverableItems:
The script will target the hidden mailbox dumpter and folders (RecoverableItemsRoot), which is useful for mailboxes under litigation hold.

.PARAMETER ScriptAction
ScriptAction is a [ValidateSet] parameter and will only accept 4 possible values:
...
When selecting an action with Export, the script will prompt where the PST Export files should be stored.  The script will allow 2 choices:
...
--------------------------
Script will prompt for the Server and Share Name and allows the operator to select the folder to be used

.PARAMETER Uri
Sets the Uri for the Exchange Web Services endpoint.  Useful when you can't leverage Autodiscover or Autodiscover fails.

.EXAMPLE
LargeItemChecks.ps1 -ServiceAccountName <User@domain.com> -ServicePassword <Password> -ItemSizeLimit <Value in MB> -MailboxLocation Primary -ScriptAction ReportOnly

-- CREATES CSV REPORT OF MAILBOXES WITH LARGE ITEMS --

In this example, the -MailboxLocation is set to Primary and the -ScriptAction has been set to ReportOnly which will create a CSV file containing all the item violations from all the mailboxes that were scanned. This CSV Report can be used in a subsequent execution where an alternate -ScriptAction is used (e.g. MoveLargeItems or MoveAndExportItems).  This may save considerable execution time if the script is run multiple times.

.EXAMPLE
LargeItemChecks.ps1 -ServiceAccountName <User@domain.com> -ServicePassword <Password> -ItemSizeLimit <Value in MB> -MailboxLocation Archive -ScriptAction ReportOnly
...
.EXAMPLE
LargeItemChecks.ps1 -ServiceAccountName <User@domain.com> -ServicePassword <Password> -ItemSizeLimit <Value in MB> -MailboxLocation RecoverableItems -ScriptAction ReportOnly
...
.EXAMPLE
LargeItemChecks.ps1 -ServiceAccountName <User@domain.com> -ServicePassword <Password> -ItemSizeLimit <Value in MB> -ScriptAction ExportLargeItems

-- EXPORT LARGE ITEMS ONLY --

In this example, the script is not using the -MailboxLocation parameter because the mailbox will not be searched using EWS.  This ScriptAction will only attempt to create a New-MailboxExportRequest and export the contents of the Large Item folder to a PST.
...
[EXCHANGE 2010/2013/2016 PERMISSIONS]
...
When specifying an Export ScriptAction, please ensure that the network location has NTFS Read/Write permissions for the "Exchange Trusted Subsystem" Group
...
.LINK
Exchange 2010 / 2013 / 2016 Manage Role Groups:  https://technet.microsoft.com/en-us/library/jj657480(v=exchg.160).aspx
#>
...
Function Get-FolderItems {
    ...
    foreach ($Folder in $Folders) {
        ...
        Do {
            ...
            $LargeItems = $null
            $LargeItems = $Items | ? { [Math]::Round(($_.Size / 1000000), 2) -gt $ItemSizeLimit }
            If (($LargeItems | Measure).Count -gt 0) {
                ...
                Foreach ($Item in $LargeItems) {
                    ...
                    If ($ScriptAction -eq "MoveLargeItems" -or $ScriptAction -eq "MoveAndExportItems") {
...
execute this script has sufficient permissions for the operation!
...
PST files for Users checked using this script.  For performance purposes,
...
By selecting [A]LL MAILBOXES, the script will use the Cmdlet Get-Mailbox
...
\\\\\\\\\\  Script:   LargeItemChecks.ps1
\\\\\\\\\\
\\\\\\\\\\  Help:    Get-Help .\LargeItemChecks.ps1 -Full
\\\\\\\\\\
\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
"@
...
try {
    ...
    If (Test-Path -Path $RootRegPath'\ExchangeServer\v15\AdminTools') {
        ...
        Write-Log -Type ERROR -Text ("Microsoft Exchange Server Management Tools cannot be found or are not installed, Script will now exit")
        Return
    }
    ...
    If (-NOT (Get-Command Get-ExchangeServer -ErrorAction SilentlyContinue)) {
        ...
        If (Test-Path $env:ExchangeInstallPath'bin\RemoteExchange.ps1') { 
            ...
            Write-Log -Type INFO -Text ("Microsoft Exchange Server Management Shell could not be loaded, Script will now exit")
            Return
        }
    } Else {
        $Script:CurrentPSSession = Get-PSSession -InstanceId (Get-OrganizationConfig).RunspaceId.Guid
    ...
    Switch ($ScriptAction) {
        "ReportOnly" {
            Write-Log -Type INFO -Text ("SCRIPT ACTION: {0}" -f $ScriptAction)
        ...
        "MoveAndExportItems" {
            ...
            Switch ($Host.UI.PromptForChoice("`n >> PST EXPORT LOCATION <<", "`n Which type of location do you want to store the PST Export Files?`n`r", $Options, 1)) {
                0 {
                    ...
                    If ((Get-ChoicePrompt -OptionList "&OK", "&Cancel" -Title " OK to Continue, Cancel to exit the script`n`r" -Message $null -default 1) -eq 1) {
    ...
    Switch ($Host.UI.PromptForChoice("`n >> MAILBOX SELECTION <<", "`n Please select which Mailboxes to peform Large Item Checks?`n`r", $Options, 0)) {
        ...
        1 {
            ...
            Do {
                ...
                If ([System.String]::IsNullOrEmpty($ImportFile)) {
                    Write-Log -Type ERROR -Text ("No File was selected - Script is exiting")
                    Return
                } Else {
        ...
        2 {
            ...
            Switch (Get-ChoicePrompt -OptionList "&OK", "&Cancel" -Title " OK to Continue, Cancel to exit the script`n`r" -Message $null -default 1) {
                0 {
                    If ((Get-Job).Count -gt 0) {
                        Write-Log -Type WARNING -Text ("Multiple Jobs exists, running Remove-Job...")
                        Sleep -Milliseconds 2250
                        Get-Job | Remove-Job
                    }
                    If (Get-PSSession | ? { $_.ConfigurationName -eq "Microsoft.Exchange" -and $_.State -eq "Opened" }) {
                        ...
                        If ((Get-Job -Name GetMailboxes).State -eq "Failed") {
                            ...
                            $Script:MailboxObjects = Get-Mailbox -ResultSize Unlimited -WA SilentlyContinue | ? { $_.PrimarySMTPAddress -notlike "extest*" } | Select PrimarySMTPAddress
                            ...
                            $MailboxObjects = $MailboxObjects | ? { $_.PrimarySMTPAddress -notlike "extest*" } | Select PrimarySMTPAddress
                            If (($Script:MailboxObjects | Measure).Count -gt 0) { Get-Job -Name GetMailboxes | Remove-Job }
                            Write-Log -Type INFO -Text ("Mailbox Objects to process:  {0:N0}" -f $MailboxObjects.Count)
                            Sleep -Milliseconds 2250
                        } Else {
...
"@)
    ...
    $i = 0
    $Count = ($MailboxObjects | Measure).Count
    Foreach ($Mailbox in $MailboxObjects) {
        If ($Count -gt 1) { Write-Progress -Id 7 -Activity ("Current User: {0}" -f $Mailbox.PrimarySMTPAddress) -Status ("[ACTION: {0}] Processing Mailboxes..." -f $ScriptAction) -CurrentOperation ("Mailbox {0} of {1}" -f ($i + 1), $Count) -PercentComplete (($i / $Count) * 100) }
        ...
        If ($ScriptAction -eq "ExportLargeItems") {
            ...
            If ($MailboxFolders.DisplayName -contains $LargeFolderName) { $MailboxFolders = $MailboxFolders | ? { $_.DisplayName -ne $LargeFolderName } }
            $MailboxLargeItems = Get-FolderItems -ItemSizeLimit $ItemSizeLimit -Folders $MailboxFolders -Service $EWSService
            If (($MailboxLargeItems | Measure).Count -gt 0) {
                ...
                $MailboxLargeItems | % { [Void]$Script:AllLargeItems.Add($_) }
                If ($ScriptAction -eq "MoveAndExportItems") {
                    If ($CentralizedExport) { Export-LargeItems -Identity $Mailbox.PrimarySMTPAddress -Path $ExportPath -FolderName $LargeFolderName }
                    Else { Export-LargeItems -Identity $Mailbox.PrimarySMTPAddress -Path $ADInfo.homedirectory -FolderName $LargeFolderName }
                }
            }
    ...
    If (($Script:AllLargeItems | Measure).Count -gt 0) { $Script:AllLargeItems | New-LargeItemReport }
} Catch {
    Write-Debug "Catch block"
    Write-Log -Type ERROR -Text ("Script terminated unexpectedly, details saved to $ScriptPath\ErrorOutput.txt") -Verbose
    ...
    Show-Menu -Title "Script Execution Ended!" -DisplayOnly -Style Mini -Color Green
    ...
    Write-Log -Type INFO -Text ("Script completed | $ElapsedTime") -Verbose
} 
